...
export default function Home() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    else setError("We couldn’t send your request just now. Please try again in a moment.");
  }
  return <>
    ...
    <main id="top">
      ...
      <section id="quote" className="quote section"><div><p className="eyebrow">THE ORIGINAL PART IS BEST</p><h2>Let&apos;s figure out your replacement.</h2><p>Sending us the broken part gives us the best chance of a precise, reliable fit. If you can&apos;t send it, photos and a link to the item it came from help us get started.</p><aside><strong>A quick note on fit & safety</strong><br/>We don&apos;t make parts for safety-critical, electrical, plumbing-pressure, fire-protection, security, or medical uses.</aside></div><form onSubmit={submit}>{submitted?<div className="success"><b>✓</b><h3>Request sent.</h3><p>Thanks—we&apos;ll review your part and get back to you with next steps.</p><button type="button" onClick={()=>setSubmitted(false)}>Send another request</button></div>:<><label>Your name<input required name="name" placeholder="Jane Smith"/></label><label>Email or phone<input required name="contact" placeholder="you@example.com"/></label><label>Link to the item<input name="itemLink" type="url" placeholder="Store page, manufacturer page, or product listing"/></label><label>What part do you need?<textarea required name="details" placeholder="What broke? What does it belong to? Any measurements or details you know?"/></label><label className="upload">Add photos <small>Up to 4 MB total. Or send the actual broken part for the best fit.</small><input name="photos" type="file" accept="image/*" multiple/></label>{error && <p className="formError" role="alert">{error}</p>}<button className="button" type="submit" disabled={sending}>{sending ? "Sending…" : "Request a quote →"}</button><small>No obligation. We&apos;ll confirm what&apos;s possible before any work begins.</small></>}</form></section>
